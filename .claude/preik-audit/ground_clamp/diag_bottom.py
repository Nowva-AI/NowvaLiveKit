...
for seed in range(8):
    ...
    for i in range(len(gen["s"])):
        ...
        if gen["phase"][i] == "bottom" and ground.is_calibrated:
...
ideal_at = gen["world"][gen["phase"] == "bottom"]
print("true ankle-above-toe at bottom (cm):", 100 * np.mean(ideal_at[:, CK.LEFT_FOOT_INDEX, 1] - ideal_at[:, CK.LEFT_ANKLE, 1]))
